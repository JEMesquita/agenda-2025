...
if ($_POST) {
    ...
    try {
        ...
        $sucesso = 'SAP cadastrado com sucesso!';
    } catch (Exception $e) {
...
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $stmt = $pdo->prepare('DELETE FROM sap WHERE id = ?');
    $stmt->execute([$id]);
    header('Location: add_sap.php');
    exit;
}
...
if (isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    $stmt = $pdo->prepare('SELECT * FROM sap WHERE id = ?');
    $stmt->execute([$id]);
    $editRow = $stmt->fetch();
}
...
if (isset($_POST['update'])) {
    ...
    $contato = trim($_POST['contato']);
    $email = trim($_POST['email']);
    $stmt = $pdo->prepare('UPDATE sap SET nome = ?, setor = ?, contato = ?, email = ? WHERE id = ?');
