...
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $stmt = $pdo->prepare('DELETE FROM cgd WHERE id = ?');
    $stmt->execute([$id]);
    header('Location: add_cgd.php');
    exit;
}
...
if (isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    $stmt = $pdo->prepare('SELECT * FROM cgd WHERE id = ?');
    $stmt->execute([$id]);
    $editRow = $stmt->fetch();
}
...
if (isset($_POST['update'])) {
    ...
    $contato = trim($_POST['contato']);
    $email = trim($_POST['email']);
    $stmt = $pdo->prepare('UPDATE cgd SET nome = ?, setor = ?, contato = ?, email = ? WHERE id = ?');
...
<body>
    <div class="container">
        <h2>Cadastrar Contato da CGD</h2>
